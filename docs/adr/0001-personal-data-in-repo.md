...
Tomorrow is a single-user tool. Defaults, Templates, Checklists, weather location, and the live Session file (`data/session.json`) all live under the repo (e.g. `data/`) rather than a home-directory dotdir or a database. The Session file is gitignored — it stays on disk in the clone so an unfinished Session can resume, but it is tonight's scratch, not a versioned library. The finished Plan HTML is not personal data kept here: it is output, written to the Desktop where it will be seen.
